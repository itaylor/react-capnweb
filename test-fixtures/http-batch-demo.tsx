...
function ApiTests() {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      const echoResult = await api.echo('Hello HTTP Batch');
      ...
      const userData = await api.getUserData('test-user-123');
      ...
      const r1 = await api.add(1, 2);
      const r2 = await api.add(3, 4);
      const r3 = await api.add(5, 6);
      if (r1 === 3 && r2 === 7 && r3 === 11) {
      ...
      // Test 6: Concurrent calls (batching test)
...
function ManualSessionControl() {
  ...
  return (
    <div className='test-section'>
      ...
      <div className='info-box'>
        ℹ️ HTTP Batch has no persistent connection. Calling close() disposes the
        session and prevents further RPC calls.
      </div>
      <button
        className='action-button'
        onClick={handleClose}
      ...
      {closed && (
        ...
        >
          Session closed. Further RPC calls will fail.
        </div>
      )}
    </div>
  );
